...
3. **Rollback runs newest-first and is failure-tolerant by default.** `runRollback` iterates the ops array in reverse, collecting throwing operations into `result.failures` and continuing. `stopOnFailure: true` halts at the first failure and returns the older, un-run operations in `result.pending` (in registration order). Rollbacks always run sequentially — never parallelize the loop; concurrency is the caller's job inside a single `add`.
